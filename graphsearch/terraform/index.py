...
from graphsearch.terraform.parse import ParseResult, parse_root


def index_terraform(config: GraphsearchConfig, session: Session, repo_root: Path) -> None:
    ...
    3. Extract :class:`~graphsearch.model.TerraformModule` and
       :class:`~graphsearch.model.Resource` nodes plus ``CALLS``,
       ``DECLARES``, and ``DEPENDS_ON`` edges.
    4. Upsert every node and edge idempotently via the Bolt session.

    Args:
        config:     Loaded ``.graphsearch.toml`` configuration.
    ...
    for root_rel in config.terraform_roots:
        ...
        if not root_dir.is_dir():
            print(f"graphsearch index: terraform root not found, skipping: {root_rel}")
            continue

        print(f"graphsearch index: parsing terraform root {root_rel!r} ...")
        ...
        print(
            f"graphsearch index: {root_rel!r} — "
