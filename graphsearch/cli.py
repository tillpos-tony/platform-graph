...
def cmd_index(args: argparse.Namespace) -> int:
    ...
    if config is None:
        print(
            "graphsearch index: no .graphsearch.toml found. "
            "Run `graphsearch init` first.",
            file=sys.stderr,
        )
        return 1
    ...
    if not config.terraform_roots and not config.k8s_overlays:
        print("graphsearch index: nothing to index (no terraform_roots or k8s_overlays configured).")
        return 0
...
def cmd_query(args: argparse.Namespace) -> int:
    ...
    if config is None:
        print(
            "graphsearch query: no .graphsearch.toml found. "
            "Run `graphsearch init` first.",
            file=sys.stderr,
        )
        return 1
